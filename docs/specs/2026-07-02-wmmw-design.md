...
- **One organism, not two sims (user-decided; reworked 2026-07-04 to ONE simulation).** The 2026-07-02 build merged hero sim + filament onto one canvas but kept two coupled fields (upscaled hero grid + sprite-stamped ribbon); user feedback: the two textures read as glued at the hero seam. Superseded by a SINGLE physarum simulation in `src/lib/mycelium.ts`: one low-res trail grid (5px/cell) spans the whole document, one agent colony, one warm umber-to-gold LUT. Per page only the pre-deposited GUIDE FIELD differs: hero zone (meandering left lane + funnel into the trunk mouth; spill 0.6, identical on every page since heroes are uniform) and, below the hero, guide tubes along the seeded trunk path + branches pouring into each panel header. Scroll gating = a frontier row (lerp toward scrollY+62%vh, monotonic per page) agents cannot cross; a small colony share re-emerges at the growth tip / on starved guide cells so colonization keeps pace with scroll. Budget: only the viewport band (±280px, in grid rows) is stepped/diffused/re-rendered per frame; outside rows freeze with their trail intact. Anti-artefact rules learned: repellent sensing margin at walls + absorbing boundary columns in the blur (else wall-hugging feedback paints a solid bar); softer deposit cap + render gain below the hero (strand, not pipe); overcrowd thinning redistributes agents from saturated cells to starved branches. Navigation carries the hero-zone trail by 1:1 CROP (never vertical resample - squish = streaky banding), slightly decayed.
- **Hero on every page, persistent across navigation (2026-07-04: SAME SIZE everywhere, copy on the RIGHT).** `<ClientRouter />` everywhere; the mycelium canvas persists across swaps (the bloom never restarts; below-hero channels re-grow per page after re-measuring anchors on `astro:page-load`). Every hero is 100dvh (homepage is the reference); variants differ only in copy: home = `full` (thesis), other pages = `band` (page-title h1, BaseLayout `heroVariant` / `heroTitle`). The copy block is anchored bottom-RIGHT (away from the left-weighted bloom), with the scrim reaching leftward toward the incoming mass. Hero.astro owns copy + veil only (no canvas); one shared veil gives the bottom fondu but feathers open over the left column so the trunk crosses unwalled. Replaces "all pages except / skip the hero sim" (§3) and the zero-client-JS post budget (§4).
...
- **Tip easing into the footer** (2026-07-04): past ~72% scroll progress the growth-tip line eases from 62% vh down to the viewport bottom, so the trunk completes into the footer (and the footer terminal's `myc:reach` fires) while the end of the page is being read, never after.
